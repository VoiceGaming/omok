...
class Omok:
    def __init__(self, root, model):
        ...
        self.root.after(800, self.state_machine)
    ...
    def voice_input_thread(self):
        ...
        self.root.after(0, lambda: self.handle_voice_input(result))
        ...
        if yes_or_no_or_error != ERROR:
            if yes_or_no_or_error == YES:
                if self.place_stone_by_voice(self.row, self.col):
                    ...
                    self.state_label.config(text="Invalid Coordinate. Try again...")
                    self.flg = True
                    self.state = VOICE_INPUT
        ...
        if self.flg:
            if self.state == LOADING:
                ...
                if self.check_winner(self.row, self.col):
                    ...
                    self.state_label.config(text="Do you want to play again? (Yes/No)")
                    self.flg = False
